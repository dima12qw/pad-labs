defmodule KV.Server do
  ...
  def start(_type, _args) do
    ...
    children = [
      ...
      worker(Task, [KV.Server, :accept, [4040]]),
      worker(KV.Router.Statistics, [KV.Router.Statistics])
    ]
  ...
  defp serve(socket) do
    msg =
      with {:ok, data} <- read_line(socket),
           {:ok, command} <- KV.Server.Command.parse(data),
           do: KV.Server.Command.run(command)
